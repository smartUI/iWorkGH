...
class Redis3 extends Cache {
    ...
    public function __construct($options = array()) {
        ...
        if (empty($options)) {
            $options = array(
                'host' => C('DATA_REDIS_HOST3') ? C('DATA_REDIS_HOST3') : '127.0.0.1',
                'port' => C('DATA_REDIS_PORT3') ? C('DATA_REDIS_PORT3') : 6379,
                'timeout' => C('DATA_CACHE_TIME3') ? C('DATA_CACHE_TIME3') : false,
                'persistent' => C('DATA_PERSISTENT3') ? C('DATA_PERSISTENT3') : false,
                'auth' => C('DATA_REDIS_AUTH3') ? C('DATA_REDIS_AUTH3') : false,
            );
        }
        ...
        $this->options['expire'] = isset($options['expire']) ? $options['expire'] : C('DATA_EXPIRE2');
        $this->options['prefix'] = isset($options['prefix']) ? $options['prefix'] : C('DATA_CACHE_PREFIX3');
    ...
    public function lrem($name,$value, $isPrefix=false){
        self::connect(true);
        $key = $isPrefix ? $this->options['prefix'] . $name : $name;
        return $this->handler->lRem($key,$value,0);
    }
}
