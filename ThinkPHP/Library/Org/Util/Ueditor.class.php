...
class Ueditor{
	...
	private $rootpath = '/content';
	
	public function __construct($uid = ''){
		//uid 为空则导入当前会话uid
		if(''===$uid) $this->uid = session('admin_id');
	...
	private function uploadFile($config,$fieldName){
        ...
        $info = $upload->uploadOne($_FILES[$fieldName]); //上传到本地
        $local_url = $this->rootpath.$info['savepath'].$info['savename']; //拼接本地图片地址

        //$rootpath = $this->rootpath;
        //图片物理地址
        $ph_url = realpath(__ROOT__).$local_url;

		self::thumbimage($ph_url);

        //同时为七牛上传
        //-----------------------
        set_time_limit(0);
        $qiniu = new \Vendor\Qiniu();
        $info = $qiniu->upload(substr($local_url,1), $ph_url);
        //dump($info);die;
		...
		if(!$info){
            ...
            $size = GetImageSize($ph_url);
			$data = array(
				'state'    => "SUCCESS",
                //'url'      => C('IMG_HOST').$local_url,
                'url'      => $local_url,
                'title'    => $info['savename'],
	...
	private function thumbimage($filename)
	{
	...
	private function saveRemote($config , $fieldName){
		...
		foreach ($source as $imgUrl) {
        	...
        	if(\Vin\FileStorage::put($rootpath,$filename,$img)){
                //---------------------------
                //$imgname = $m ? $m[1] : $imgname;
                $file_name = $rootpath.$path.$imgname;
                $ph_url = realpath(__ROOT__).$file_name;
                //同时为七牛上传
                set_time_limit(0);
                $qiniu = new \Vendor\Qiniu();
                $info = $qiniu->upload(substr($file_name,1), $ph_url);
                $size = GetImageSize($ph_url);
