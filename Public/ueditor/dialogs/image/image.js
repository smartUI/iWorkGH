...
(function () {
    ...
    RemoteImage.prototype = {
        ...
        setPreview: function(){
            ...
            if(url) {
                preview.innerHTML = '<img src="' + url + '" width="' + width + '" height="' + height + '"  title="' + title + '" id="' + imgid + '"/>';
				
				//添加图片点击start
				var imgwidth = $("#"+imgid).width();
				var imgheight = $("#"+imgid).height();
				var imgpercent = imgwidth/imgheight;
				if(imgheight >= 360){
					imgheight = 360;
					imgwidth = 360 * imgpercent;
				}
				if(imgwidth >= 600){
					imgwidth = 600;
					imgheight = 600/imgpercent;
				}
				
				$("#"+imgid).click(function(){					
					var popdiv ="<div class='addTagpop'><div class='addtagdiv'></div><img src='" + url + "' id='tag" + imgid + "' style='margin-left:-"+(imgwidth/2)+"px;margin-top:-"+(imgheight/2)+"px;' class='addtagdivimg' width='"+imgwidth+"' height='"+imgheight+"' /><a  class='addtagsubmit'  href='javascript:;'>保存</a><a class='addtagcolse' href='javascript:;'></a></div>";	
					$("body").append(popdiv);
					$("body").delegate("a.addtagcolse","click",function(){
						$(this).parent().remove();	
					})						
					...
					//点击保存提交打点数据
					$("body").delegate("a.addtagsubmit","click",function(){
						var dataworlds = new Array;
						$(".jTagTag").each(function(){
							var getTagitem = $(this).attr("data-base");
							dataworlds.push(getTagitem);

						})
						//console.log(dataworlds);
                        imgJson = {'name':JSON.stringify(dataworlds)};
                        
                        $.ajax({
                            type:"post",
                            dataType:"json",
                            data:{"name": JSON.stringify(dataworlds)},
                            async:false,
                            url:"/Home/News/newsImgPoint",
